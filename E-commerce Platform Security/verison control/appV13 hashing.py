...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password'], password):
            # Login successful set cookie and go home (change to session later)
...
def product_detail(product_id):
    ...
    if reviews:
        ...
        for review in reviews:
            ...
            html += f"<li><strong>{review['author_name']}</strong>: {rendered_comment}"

            # If this review has an image URL, show it under the text
            if review['image_url']:
                html += (
                    "<br><img src='"
                    + review['image_url']
...
def add_review(product_id):
    ...
    conn.execute(
        ...
        "VALUES (?, ?, ?, ?)",
        (product_id, username, comment, image_url or None))
