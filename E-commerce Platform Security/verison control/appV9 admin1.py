...
def admin_dashboard():
    ...
    conn = get_db_connection()
    users = conn.execute(
        "SELECT username, is_admin, is_seller, is_suspended FROM users ORDER BY username"
    ).fetchall()
    conn.close()
    ...
    if not users:
        ...
        for u in users:
            ...
            role_text = ", ".join(roles)
            suspended_text = " (SUSPENDED)" if u['is_suspended'] else ""

            html += (
                f"<li><a href='{user_link}'>{u['username']}</a> "
                f"- {role_text}{suspended_text}</li>"
            )
...
def admin_view_user(username):
    ...
    html += f"<li>is_suspended: {user_row['is_suspended']}</li>"
    html += "</ul>"
